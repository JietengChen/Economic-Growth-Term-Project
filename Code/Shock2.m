%%%% In this file we introduce an external shock lasting for TWO year
...
wage_shock(1)=0;
 for t=1:1:T   
     ...
     if t==27
         A2_shock(t+1)=0.95*A2_shock(t-1);
         A3_shock(t+1)=0.9*A3_shock(t-1);
     end
     if t==28
         A2_shock(t+1)=A2_shock(t-2);
         A3_shock(t+1)=A3_shock(t-2);
     end 
 end
 ...
 plot( time,Y3_shock,'Color',[0.47 0.54 0.54],"LineWidth",1);
  plot( time,Y1_shock,'Color',[0 0.4 0.54],"LineWidth",1);
 legend("Agricultural Sector","Industrial Sector without Shock","Service Sector without Shock","Industrial Sector with Shock","Service Sector with Shock","Agricultural Sector with shock")
